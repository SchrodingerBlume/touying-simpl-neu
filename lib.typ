// This theme is based on https://github.com/tzhtaylor/touying-sjtu
// which is inspired by https://github.com/zbowang/BeamerTheme and https://github.com/touying-typ/touying/blob/main/themes/dewdrop.typ
  ...
  if self.store.navigation == "sidebar" {
    ...
    grid(
      ...
      grid(
        ...
        align(right + horizon, image("vi/neu-vi-neumotto.png", height: 0.9cm)),
      ),
    ...
    grid(
      ...
      grid(
        columns: (75%, 25%),
        align(left + horizon, utils.display-current-heading(depth: self.slide-level, style: auto)),
        align(right + horizon, image("vi/neu-vi-neumotto.png")),
      ),
      align(center + horizon, line(length: 100%, stroke: (paint: self.colors.primary, thickness: 1.5pt))),
    )
  }
...
#let slide(
  ...
  let self = utils.merge-dicts(
    self,
    config-page(header: neu-header, footer: neu-footer, margin: (x: 1.25cm)),
...
#let title-slide(
  ...
  self = utils.merge-dicts(self, config, config-common(freeze-slide-counter: true), config-page(
    header: align(right + horizon, block(inset: (right: 0.3em, top: 0.3em), image("vi/neu-vi-logo.png", height: 3em))),
  ...
  let body = {
    ...
    set page(background: align(left + bottom, image("vi/neu-vi-neubg.png", width: if self.show-notes-on-second-screen
      == right { 50% } else { 100% })))
...
#let title-slide-blue(
  ...
  let body = {
    ...
    set page(background: align(left, image("vi/neu-vi-neuphoto.jpg")))
    ...
    block(fill: self.colors.primary, width: 100%, {
      ...
      grid(
        ...
        align(right, block(inset: (x: 2em), image("vi/neu-vi-logo-white.png"))),
      )
      v(2em)
    })
  }
...
#let outline-slide(config: (:), title: utils.i18n-outline-title, ..args) = touying-slide-wrapper(self => {
  self = utils.merge-dicts(self, config-common(freeze-slide-counter: true), config-page(footer: neu-footer, margin: (
    top: 3em,
  )))
  ...
  self = utils.merge-dicts(self, config-page(
    header: grid(
      ...
      grid(
        ...
        align(right + horizon, image("vi/neu-vi-neumotto.png", height: 1.5cm)),
      ),
      v(-2cm),
      align(center + horizon, line(length: 100%, stroke: (paint: self.colors.primary, thickness: 1.5pt))),
    ),
...
#let end-slide(config: (:), body) = touying-slide-wrapper(self => {
  ...
  let body = {
    ...
    block(width: 80%, grid(
      columns: (40%, 1fr),
      column-gutter: 0pt,
      image("vi/neu-vi-logo-ud.png"), body,
    ))
  }
  touying-slide(self: self, config: config, align(horizon + center, body))
})
...
#let end-slide-blue(config: (:), body) = touying-slide-wrapper(self => {
  ...
  let body = {
    set page(background: align(left, image("vi/neu-vi-end.png")))
    v(3.6em)
    body
  }
...
///
/// ```typst
/// #show: neu-theme.with(aspect-ratio: "16-9", config-colors(primary: blue))`
/// ```
///
/// The default colors:
///
...
#let neu-theme(
  ...
  primary: rgb("284093"),
  // 官网说校色是 C100 M70，对应 #004c9c
  // 但实际给的示例颜色是 #284093
  // https://www.neu.edu.cn/xygk/ddbs.htm
  alpha: 40%,
